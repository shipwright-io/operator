...
func ReconcileCertManager(ctx context.Context, crdClient crdclientv1.ApiextensionsV1Interface, client client.Client, logger logr.Logger, namespace string) (bool, error) {
	...
	if err != nil {
		return true, fmt.Errorf("error creating inital certificates manifest: %v", err)
	}
	...
	if err != nil {
		return true, fmt.Errorf("error transorming manifest using target namespace: %v", err)
	}
